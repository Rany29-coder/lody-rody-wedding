...
export default function Hero() {
  return (
    <section className="relative z-10 flex min-h-dvh flex-col items-center justify-center px-5 py-16">
      ...
      >
        {/* Still, calm card — no idle motion (the couple's request) */}
        <div className="relative overflow-hidden rounded-2xl bg-[#fffdf8]/90 px-7 py-12 text-center shadow-[0_40px_90px_-35px_rgba(150,110,45,0.5)] ring-1 ring-gold/30 backdrop-blur-sm sm:px-12">
          <div className="pointer-events-none absolute inset-3 rounded-xl ring-1 ring-gold/20" />
          ...
          <Stagger delay={0.7}>
            <p className="mx-auto mt-8 max-w-sm font-serif text-base italic leading-relaxed text-foreground/85">
...
function Flourish({ className = "" }: { className?: string }) {
  return (
    <svg
      aria-hidden
      viewBox="0 0 40 40"
      className={`pointer-events-none absolute h-9 w-9 text-gold/50 ${className}`}
      fill="none"
      stroke="currentColor"
      strokeWidth="1"
    >
